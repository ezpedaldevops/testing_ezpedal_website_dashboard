...
interface CareerPageContactFormI {
  _id: string;
  first_name: string;
  last_name: string;
  email: string;
...
const Page = () => {
  ...
  const [applications, setApplications] = useState<CareerPageContactFormI[]>([]);
  const [showPopUp, setShowPopUp] = useState(false);
  const [selectedApplication, setSelectedApplication] = useState<CareerPageContactFormI | null>(null);
...
const fetchApplications = useCallback(async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/v2/career/contact/get-all`, {
...
useEffect(() => {
  ...
  const handleDelete = async (applicationId: string) => {
    ...
    try {
      await axios.delete(
        `${baseUrl}/api/v2/career/contact/delete/${applicationId}`,
        {
          headers: { Authorization: token },
        }
      );
    ...
    try {
      const res = await axios.get(`${baseUrl}/api/v2/career/contact/get/${id}`, {
        headers: { Authorization: token },
      });
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-100">
        <Head>
          <title>Career/contact-us - Ezpedal</title>
        ...
        <main className="w-full px-4 py-8 flex flex-col">
          <section>
            <div className="flex flex-col justify-center items-center mb-8">
              <h3 className="text-lg font-poppins font-medium text-black">List</h3>
              <h1 className="text-5xl font-outfit text-black">
                Career Contact Us Applications
              </h1>
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50 animate-fadeIn">
            <div className="bg-white/90 font-poppins p-8 w-[600px] rounded-2xl shadow-2xl relative animate-scaleIn border border-gray-200">
              ...
              <div className="space-y-4 max-h-[55vh] overflow-y-auto pr-2">
                ...
                <DetailRow
                  label="Topic"
                  value={selectedApplication.topic || "-"}
                />

                <DetailRow
                  label="Designation"
                  value={selectedApplication.describe_yourself || "-"}
                />
